...
g = Gdoc(api_username=dhl, api_password=<api_password>, username=DHL, password=<password>)
...
Usage (command line):
```
gdoc-dlx [-h] --dlx_connect DLX_CONNECT --s3_key_id S3_KEY_ID --s3_key S3_KEY --s3_bucket S3_BUCKET 
    --gdoc_api_username GDOC_API_USERNAME --gdoc_api_password GDOC_API_PASSWORD --gdoc_username 
	GDOC_USERNAME --gdoc_password GDOC_PASSWORD --station {NY,GE} [--date DATE] [--symbol SYMBOL]
    [--language {A,C,E,F,R,S,O}] [--overwrite]
